  ...
  const FormFields = ({ isPending, onSubmit, submitLabel }: { isPending: boolean; onSubmit: () => void; submitLabel: string }) => (
    <div className="space-y-4 pt-2">
      <div>
        ...
        <Input className="mt-1" value={form.title} onChange={(e) => set("title", e.target.value)} placeholder="e.g. Valley Ridge Hike — Summer 2024" />
      </div>
      <div>
        <Label>Event Type <span className="text-destructive">*</span></Label>
        <Input className="mt-1" value={form.eventType} onChange={(e) => set("eventType", e.target.value)} placeholder="e.g. Hiking, Cycling, Night Walk" />
      </div>
      <div>
        <Label>Short Description <span className="text-destructive">*</span></Label>
        <Textarea className="mt-1 min-h-[80px]" value={form.shortDescription} onChange={(e) => set("shortDescription", e.target.value)} placeholder="A brief summary shown on the public page" />
      </div>
      <div>
        <Label>Cover Image URL <span className="text-muted-foreground">(optional)</span></Label>
        <Input className="mt-1" value={form.coverImageUrl} onChange={(e) => set("coverImageUrl", e.target.value)} placeholder="https://..." />
        {form.coverImageUrl && (
          <img src={form.coverImageUrl} alt="preview" className="mt-2 rounded-lg w-full h-28 object-cover border" onError={(e) => { (e.target as HTMLImageElement).style.display = "none"; }} />
        )}
      </div>
      <div className="grid grid-cols-2 gap-3">
        <div>
          <Label>Event Date <span className="text-muted-foreground">(optional)</span></Label>
          <Input className="mt-1" type="date" value={form.eventDate} onChange={(e) => set("eventDate", e.target.value)} />
        </div>
        <div>
          <Label>Display Order</Label>
          <Input className="mt-1" type="number" min={0} value={form.displayOrder} onChange={(e) => set("displayOrder", e.target.value)} placeholder="0" />
        </div>
      </div>
      <div>
        <label className="flex items-center gap-2 cursor-pointer">
          <input type="checkbox" checked={form.isVisible} onChange={(e) => set("isVisible", e.target.checked)} className="accent-primary" />
          <span className="text-sm">Visible on registration pages</span>
        </label>
      </div>
      <div className="flex gap-3 pt-2">
        <Button className="flex-1" onClick={onSubmit} disabled={isPending || !form.title.trim() || !form.eventType.trim() || !form.shortDescription.trim()}>
    ...
    <Layout>
      <div className="container py-12">
        ...
        {isLoading ? (
              ...
              <div key={e.id} className="border rounded-xl overflow-hidden bg-card shadow-sm">
                ...
                <div className="p-4">
                  ...
                  <div className="flex items-center gap-1.5">
                    ...
                    <Button variant="ghost" size="sm" className="h-7 w-7 p-0 text-destructive hover:text-destructive" onClick={() => handleDelete(e.id)}>
          ...
          <div className="text-center py-24 border rounded-2xl bg-card">
            ...
            <Button onClick={() => { setForm(emptyForm); setCreateOpen(true); }}><Plus className="w-4 h-4 mr-2" /> Add your first past event</Button>
          </div>
        )}
      </div>
      ...
      <Dialog open={createOpen} onOpenChange={(o) => { if (!o) { setCreateOpen(false); setForm(emptyForm); } }}>
        <DialogContent className="max-w-md max-h-[90vh] overflow-y-auto">
          ...
          <FormFields isPending={createMutation.isPending} onSubmit={handleCreate} submitLabel="Add event" />
        </DialogContent>
      </Dialog>
